...
int pipe_write(void* pipecb_t, const char *buf, unsigned int size){
  ...
  Pipe_CB* pipe_cb = (Pipe_CB*)pipebc_t;

    //check if we can write to the buffer, if size>=1 and if buf!=Null
    if(pipe_CB!=NULL && buf!=NULL && size<1 && pipe_CB->reder!=NULL){
    return -1;
  }
  ...
  while(Flag == 1){
    ...
    // if the w_position reaches PIPE_BUFFER_SIZE, return to 0
    if (pipe_CB->w_position == (int)PIPE_BUFFER_SIZE){
